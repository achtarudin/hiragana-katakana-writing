...
<template>
	<div
		class="relative w-full h-[100dvh] bg-neutral-50 overflow-hidden select-none flex flex-row font-sans text-neutral-800">
		...
		<aside ref="navSidebarRef" :class="[
			...
			<div class="flex-1 overflow-y-auto p-4 flex flex-col gap-1.5 scrollbar-none pb-10">
				<label class="text-xs font-bold text-neutral-400 tracking-wider uppercase mb-2 px-1">Menu
					Halaman</label>
		...
		<aside ref="sidebarRef" :class="[
			...
			<div class="flex-1 overflow-y-auto p-4 flex flex-col gap-6 scrollbar-none pb-4 pt-1">
				<div class="flex flex-col gap-2 shrink-0">
					<label class="text-xs font-bold text-neutral-400 tracking-wider uppercase">Pilih Baris {{
						$route.name }}</label>
		...
		<main class="flex-1 h-full relative flex flex-col overflow-hidden">
			...
			<div class="relative flex-1 w-full h-full">
				<div :class="[
					...
					<div v-for="(item, index) in activeGroup" :key="index" :class="[
						...
						activeGroup.length <= 3 ? 'w-[28%] sm:w-[20%] md:w-[16%]' : 'w-[28%] sm:w-[25%] md:w-[15%] lg:w-[14%]'
					]">
						<span :class="[
				...
				<svg @pointerdown="handlePointerDown" @pointermove="handlePointerMove" @pointerup="handlePointerUp"
					...
					<defs v-if="isGridActive">
						<pattern id="full-canvas-grid" width="40" height="40" patternUnits="userSpaceOnUse">
							<path d="M 40 0 L 0 0 0 40" fill="none" stroke="currentColor" class="text-neutral-500/30"
								stroke-width="1" />
						</pattern>
					</defs>
			...
			<div
				...
				<div
					class="bg-neutral-900/80 text-white text-xs px-4 py-2.5 rounded-full backdrop-blur-sm pointer-events-none shadow flex items-center gap-2 shrink-0">
