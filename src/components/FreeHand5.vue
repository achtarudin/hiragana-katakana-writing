...
import { ref, computed, reactive, watch, onMounted } from 'vue' // Tambahkan onMounted di sini
...
const isSidebarOpen = ref(true) // State untuk kontrol hamburger menu
...
const options = reactive({
	size: 16, // Nilai default untuk desktop
	smoothing: 0.5,
	thinning: 0.5,
	streamline: 0.65,
	easing: (t: number) => t,
...
// DETEKSI UKURAN LAYAR OTOMATIS SAAT APLIKASI DI-LOAD
onMounted(() => {
	// Jika lebar layar kurang dari 768px (standar mobile/tablet)
	if (window.innerWidth < 768) {
		options.size = 8; // Set ukuran kuas jadi lebih proporsional untuk HP
	}
})
		...
		<main class="flex-1 h-full relative flex flex-col overflow-hidden">
			...
			<div
				...
				<button @click="prevPage" :disabled="currentIndex === 0"
					class="w-9 h-9 bg-neutral-50 border border-neutral-200 hover:bg-neutral-200 disabled:opacity-40 disabled:cursor-not-allowed rounded-lg transition-colors font-bold flex items-center justify-center">
					◀
				</button>
				...
				<button @click="undoLast" :disabled="completedLines.length === 0"
					class="px-3 h-9 bg-white border border-neutral-200 hover:bg-neutral-50 disabled:opacity-40 disabled:cursor-not-allowed text-neutral-700 font-bold rounded-lg transition-colors text-xs flex items-center justify-center">
					Undo
				</button>

				<button @click="clearCanvas" :disabled="completedLines.length === 0 && points.length === 0"
					class="px-3 h-9 bg-red-500 hover:bg-red-600 disabled:opacity-40 disabled:cursor-not-allowed text-white font-bold rounded-lg transition-colors text-xs flex items-center justify-center">
					Reset
				</button>
			</div>
				...
				<div
					...
					<div v-for="(item, index) in currentGroup" :key="index" :class="[
						...
						<span :class="[
							...
							currentIndex === index
								? 'text-[14vh] md:text-[16vh] lg:text-[20vh] text-neutral-400 font-semibold'
								: 'text-[14vh] md:text-[16vh] lg:text-[20vh] text-neutral-400/25 font-semibold'
						]">
							{{ item.kana }}
						</span>
						...
						<span :class="[
							...
							currentIndex === index
								? 'text-[4vh] md:text-[4vh] lg:text-[6vh] text-black'
								: 'text-[4vh] md:text-[4vh] lg:text-[6vh] text-neutral-400/25'
						]">
							{{ item.romaji }}
						</span>
					</div>
